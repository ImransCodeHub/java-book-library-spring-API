...
public class MainController {
     ...
     * @param isbn
     * @param copyright
     * @param edition_number - Updated to editionNumber to match the Book class
     * @param title
    ...
    @CrossOrigin(origins = "*")
    public @ResponseBody
    String updateBook(@PathVariable String isbn, @RequestParam String copyright, @RequestParam Integer editionNumber, @RequestParam String title){
        Optional<Book> book = Optional.ofNullable(bookRepository.findByIsbn(isbn));
        if(book.isPresent()){
            Book updatedBook = book.get();
            updatedBook.setCopyright(copyright);
            updatedBook.setEditionNumber(editionNumber);
